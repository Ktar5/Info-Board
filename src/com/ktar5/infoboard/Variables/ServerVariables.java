...
public class ServerVariables {
	...
	public static String replaceVariables(String string) {
		...
		if (newString.contains("<onlineplayers>"))
			newString = newString.replaceAll("<onlineplayers>",
					String.valueOf(Bukkit.getOnlinePlayers().length));
		if (newString.contains("<motd>"))
			newString = newString.replaceAll("<motd>",
					String.valueOf(Bukkit.getMotd()));
		if (newString.contains("<maxplayers>"))
			newString = newString.replaceAll("<maxplayers>",
					String.valueOf(Bukkit.getMaxPlayers()));
		if (newString.contains("<servername>"))
			newString = newString.replaceAll("<servername>",
					String.valueOf(Bukkit.getServerName()));
		if (newString.contains("<tps>"))
			newString = newString.replaceAll("<tps>",
					String.valueOf(Math.round(tps * 100.0D) / 100.0D));
		if (newString.contains("<freeram>"))
			newString = newString
					.replaceAll("<freeram>", String.valueOf((Runtime
							.getRuntime().freeMemory() / 1024) / 1024));
		if (newString.contains("<maxram>"))
			newString = newString.replaceAll("<maxram>", String
					.valueOf((Runtime.getRuntime().maxMemory() / 1024) / 1024));
		if (newString.contains("<usedram>"))
			newString = newString.replaceAll("<usedram>", String
					.valueOf(((Runtime.getRuntime().maxMemory() - Runtime
							.getRuntime().freeMemory()) / 1024) / 1024));
		if (newString.contains("<availableprocessors>"))
			newString = newString.replaceAll("<availableprocessors>",
		...
		if (newString.contains("<peoplewith")) {
			...
			for (Player ppl : Bukkit.getOnlinePlayers())
				if (ppl.hasPermission(perm))
					i++;
			newString = newString.replaceAll("<peoplewith" + (perm) + ">",
					String.valueOf(i));
		}
		return newString;
	}
